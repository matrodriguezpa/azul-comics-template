...
const latestConfig = {
  ...
  initialDisplay: 4,                  // Posts mostrados al inicio
  batchSize: 4,                       // Posts añadidos al pulsar "Ver más"
  ...
  imagenAncho: 400,                   // Ancho deseado para las imágenes (px)
...
async function processContainer(container, entries) {
  ...
  const category = section && section.id && section.id.toLowerCase() !== 'latest'
    ? section.id
    : null;
...
function optimizarImagen(url) {
  ...
  try {
    ...
    u.pathname = u.pathname.replace(
      /\/s\d+(?:-[^/]+)?\//i,
      `/w${latestConfig.imagenAncho}-h${latestConfig.imagenAlto}/`
    );
